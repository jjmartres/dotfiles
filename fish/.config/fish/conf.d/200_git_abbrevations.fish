abbr git-crypt-unlock 'git-crypt unlock $HOME/.git-crypt/git-crypt_key'
abbr lg lazygit
abbr guaf 'git_update_all fetch'
abbr guap 'git_update_all pull'
abbr goc opencommit
...
abbr gpv 'git push -v'
abbr gpomr 'git push -o merge_request.create -o merge_request.target=$(basename $(git symbolic-ref --short refs/remotes/origin/HEAD))'
...
# gs: git status
abbr gs 'git --no-pager diff --stat -B -M -C'
abbr gsa 'git status'
...
# gclb: git clean local branches
abbr gclb 'git branch | grep -v "^\*" | grep -E "(fix|feat|SPL)" | grep -v -E "\b(main|master|develop)\b" | xargs git branch -D'
